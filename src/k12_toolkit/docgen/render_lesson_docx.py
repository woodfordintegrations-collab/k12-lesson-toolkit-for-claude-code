...
# --- local addition: figures ---------------------------------------------------------
# Upstream has no image block of any kind. A geometry document that says "see fig-d03-2.svg"
# is not a deliverable, so this emitter rasterizes the SVG and places it inline.
def _emit_figure(doc, blk, theme):
    from pathlib import Path as _P
    try:                                   # package mode
        from .rasterize import rasterize as _rast
    except ImportError:                      # script mode, which is how the skill invokes it
        import sys as _sys
        _sys.path.insert(0, str(_P(__file__).resolve().parent))
        from rasterize import rasterize as _rast
    src = _P(blk.get("src", ""))
    if not src.is_absolute():
        src = _P(blk.get("_base", ".")) / src
    cap = blk.get("caption", "")
    alt = blk.get("alt", "")
    if not src.exists():
        doc.add_paragraph(f"[figure missing: {src.name}]")
        return
    cache = src.parent / "_raster"
    png, aspect = _rast(src, cache / (src.stem + ".png"))
    from docx.shared import Inches as _In
    width = float(blk.get("width_in", 5.5))
    shape = doc.add_picture(str(png), width=_In(width))
    # Word exposes only wp:docPr/@descr to a screen reader. Without it the image is
    # announced as "Graphic" and the description below is orphaned prose — so the alt
    # text is stamped on the shape itself, not only rendered as a visible paragraph.
    if alt:
        shape._inline.docPr.set("descr", alt)
        shape._inline.docPr.set("title", cap or src.stem)
    doc.paragraphs[-1].alignment = 1  # centred
    if cap:
        c = doc.add_paragraph()
        add_md(c, cap, italic=True) if "italic" in add_md.__code__.co_varnames else add_md(c, cap)
        c.alignment = 1
    if alt:
        # The long description travels with the document so a reader using a screen reader,
        # or a teacher reading aloud, is not left with an image and no words.
        a = doc.add_paragraph()
        add_md(a, f"Description: {alt}")
