...
def render_block(blk: dict, theme: Theme) -> str:
    ...
    if t == "figure":
        # Local addition: upstream has no image block. The SVG is inlined rather than
        # linked, for three reasons: the file stays self-contained (it survives being
        # emailed or hosted anywhere), it is vector so it holds up at any zoom, and the
        # figure's own <title>/<desc> reach assistive tech directly — a <desc> can carry
        # a full paragraph, which no alt attribute realistically can.
        src = Path(str(blk.get("src", "")))
        if not src.is_absolute():
            src = Path(str(blk.get("_base", "."))) / src
        cap = str(blk.get("caption", ""))
        figcap = f"<figcaption>{md(cap)}</figcaption>" if cap else ""
        if not src.exists():
            return f"<figure class=\"fig\"><p><b>[figure missing: {escape(src.name)}]</b></p>{figcap}</figure>"
        svg = src.read_text(encoding="utf-8")
        svg = re.sub(r"<\?xml[^>]*\?>\s*", "", svg)
        svg = re.sub(r"<!DOCTYPE[^>]*>\s*", "", svg)
        # Drop the physical width/height so CSS governs size; viewBox keeps the aspect.
        # Left in place, width="160mm" overflows the column on a narrow screen.
        svg = re.sub(r'(<svg\b[^>]*?)\s+width="[^"]*"', r"\1", svg, count=1)
        svg = re.sub(r'(<svg\b[^>]*?)\s+height="[^"]*"', r"\1", svg, count=1)
        if "role=" not in svg.split(">", 1)[0]:
            # Every figure must reach a screen reader as an image, never as loose markup.
            svg = svg.replace("<svg", '<svg role="img"', 1)
        return f"<figure class=\"fig\">{svg}{figcap}</figure>"
    if t == "instructions":
