...
# --- local addition: figure base paths -----------------------------------------------
def stamp_figure_base(node, base: str) -> None:
    """Stamp `_base` on every figure block so a relative `src` resolves against the
    input JSON's own directory. Without this a figure path is only valid from one
    working directory, and the checked-in JSON ends up carrying absolute machine
    paths — which is how it breaks for everyone who is not the author."""
    if isinstance(node, dict):
        if node.get("type") == "figure" and "src" in node:
            node.setdefault("_base", base)
        for value in node.values():
            stamp_figure_base(value, base)
    elif isinstance(node, list):
        for item in node:
            stamp_figure_base(item, base)
...
def main() -> int:
    ...
    stamp_figure_base(source, str(Path(args.input).resolve().parent))
